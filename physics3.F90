...
!=====================================================================
SUBROUTINE fractal_dimension(p, fracVar)
   USE the_info
   ...
   IF (p%af .eq. 0) THEN
      p%frac = fracVar !!3+(fracVar/pi)*DATAN(-p%Nn/1.0d02)
!      IF (p%s .gt. 0.1) THEN
!         p%frac = 3+(fracVar/pi)*(DATAN(-p%Nn/1.0d02)*DATAN(p%s/0.1))
!print*, p%s, p%frac, p%z
!      ENDIF
...
! the aggregate is so tiny that it is considered dissolved
   ...
   IF (p%r .lt. 0.2 .and. p%b .eq. 0) THEN
      ...
      Th_d_234(j) = Th_d_234(j) + p%Th_234*p%n
      Th_d_230(j) = Th_d_230(j) + p%Th_230*p%n
   ENDIF
...
!====================================================================
SUBROUTINE disintegrate(p, harvest, fracVar)
...
!====================================================================
SUBROUTINE stickiness(p, stickVar)
...
SUBROUTINE bottom(p, j)
   ...
   IF (p%z .gt. seafloor) THEN
      ...
      ENDIF
      seaBed(19) = seaBed(19) + p%Th_234*p%n
      Th_inv = Th_inv - p%Th_234*p%n  !Th out of water column
...
SUBROUTINE pool(p1, p2)
   ...
   p1%Th_234 = (p1%Th_234*p1%n + p2%Th_234*p2%n)/(p1%n+p2%n)
   p1%Th_230 = (p1%Th_230*p1%n + p2%Th_230*p2%n)/(p1%n+p2%n)

   p2%b = 1

END SUBROUTINE pool
!=====================================================================
SUBROUTINE radius(p, fracVar)
   USE the_info
...
!=====================================================================
SUBROUTINE collide4(p1, p2, harvest, stickVar, fracVar, paraT)
   
   USE the_info

   TYPE(agg_part), INTENT(INOUT) :: p1, p2
   DOUBLE PRECISION, INTENT(INOUT) :: harvest, stickVar, fracVar, paraT
...
! find number of j-particles to collide with i-particles
      ...
      IF (q_int .gt. 0) THEN
         CALL stick2(p1,p2,q_int, stickVar, fracVar)
      ENDIF
...
!========================================================================
SUBROUTINE stick2(p1, p2, intRate, stickVar, fracVar)
   USE the_info

   TYPE(agg_part), INTENT(INOUT) :: p1, p2
   DOUBLE PRECISION, INTENT(INOUT) :: stickVar, fracVar
   ...
   IF (p1%n .gt. p2%n .and. intRate*p2%n .lt. p1%n) THEN
      ...
      CALL radius(p2, fracVar)
      CALL density(p2, fracVar)
      CALL stickiness(p2, stickVar)
   ELSEIF (p1%n .lt. p2%n .and. intRate*p1%n .lt. p2%n) THEN
      ...
      CALL radius(p1, fracVar)
      CALL density(p1, fracVar)
      CALL stickiness(p1, stickVar)
...
!========================================================================
SUBROUTINE self_collide(p, harvest, stickVar, fracVar)
   
   USE the_info

   TYPE(agg_part), INTENT(INOUT) :: p
   DOUBLE PRECISION, INTENT(INOUT) :: harvest, stickVar, fracVar
   DOUBLE PRECISION :: beta, betaBr, betaSh, betaDS
...
!      prob = 1 - exp(-alpha*beta*sf1*timestep)
      ...
      IF (prob .gt. harvest) THEN
         CALL self_stick(p, stickVar, fracVar)
      ENDIF
   ENDIF
...
!========================================================================
SUBROUTINE self_stick(p, stickVar, fracVar)
   USE the_info
   ...
   CALL radius(p, fracVar)
   CALL density(p, fracVar)
   CALL stickiness(p, stickVar)
...
!========================================================================
SUBROUTINE break_p(p, harvest, fracVar)
   USE the_info
   ...
   IF (p%Nn .gt. daughters) THEN
      ...
      DO WHILE (i .le. 4) 
         ...
         i = i + 1
      ENDDO
      p%Th_234 = p%Th_234 * new_Nn/old_Nn
      p%Th_230 = p%Th_230 * new_Nn/old_Nn
      CALL radius(p, fracVar)
   ENDIF 
   TEP_t = 0
   i = 1
   ...
   IF (p%Th_234 .lt. 0) print*, 'break - Th', p%Th_234, id, p%Th_230
   IF (p%Th_230 .lt. 0) print*, 'break-Th230', p%Th_234, id, p%Th_230

END SUBROUTINE break_p
!========================================================================
SUBROUTINE radioactive_decay(A_Th_234, A_Th_230)
! sources minus sinks: prod from U, loss to Pa
! A_Th = activity of Th is in units of dpm/L
   USE the_info
   
   DOUBLE PRECISION, INTENT(INOUT) :: A_Th_234, A_Th_230
   DOUBLE PRECISION :: dA_Th_234, dA_Th_230

   dA_Th_234 = (A_U_238 - A_Th_234) * lambda_Th_234 * timestep
   dA_Th_230 = (A_U_234 - A_Th_230) * lambda_Th_230 * timestep

   A_Th_234 = A_Th_234 + dA_Th_234
   A_Th_230 = A_Th_230 + dA_Th_230

   IF(A_Th_234 .lt. 0) print*, 'rd:', dA_Th_234, A_Th_234
   IF(A_Th_230 .lt. 0) print*, 'rd:230', dA_Th_230, A_Th_230

END SUBROUTINE radioactive_decay
!========================================================================
SUBROUTINE radioactive_decay_particle(p)
! sources minus sinks: prod from U, loss to Pa
   USE the_info
   
   TYPE(agg_part), INTENT(INOUT) :: p
   DOUBLE PRECISION :: dTh_234, dTh_230, before, after

   dTh_234 = p%Th_234 * lambda_Th_234 * timestep
   dTh_230 = p%Th_230 * lambda_Th_230 * timestep

   p%Th_234 = p%Th_234 - dTh_234
   p%Th_230 = p%Th_230 - dTh_230

   IF (p%Th_234 .lt. 0) print*, 'radio 234:', d_Th_234, before/p%n
   IF (p%Th_230 .lt. 0) print*, 'radio 230:', d_Th_230, p%z/100

END SUBROUTINE radioactive_decay_particle
!========================================================================
SUBROUTINE adsorb(p,A_Th_234, A_Th_230)
! k_m1 = desorption constant = 0.068 1/d
! k_1 = adsorption function of particle = (1e3/radius)**1.5 [cm3/g/d]
   USE the_info
   
   TYPE(agg_part), INTENT(INOUT) :: p
   DOUBLE PRECISION, INTENT(INOUT) :: A_Th_234, A_Th_230
   DOUBLE PRECISION :: adsorbed_234, desorbed_234, adsorbed_230
   DOUBLE PRECISION :: mass, Th_k1, before, after, desorbed_230
 
! units: p%Th*p%n [dpm/part * part/box = dpm/box]
!        A_TH_23x [dpm/L]
!        Th_k1 [box/gs]

   CALL adsorbtion_constant(p,Th_k1)
   CALL particle_mass(p,mass)

   adsorbed_234 = Th_k1 * A_Th_234 * p%n*mass ![box/gs*dpm/L*g/box] = [dpm/Ls]
   desorbed_234 = Th_km1 * p%Th_234 * p%n * 1/(10*dz) ![1/s*dpm/part*part/box*box/L] = [dpm/Ls]

   adsorbed_230 = Th_k1 * A_Th_230 * p%n*mass  ![dpm/Ls]
   desorbed_230 = Th_km1 * p%Th_230 * p%n * 1/(10*dz) ![dpm/Ls]

   dTh_p_234 = (adsorbed_234 - desorbed_234) * timestep ![dpm/Lts]
   dTh_p_230 = (adsorbed_230 - desorbed_230) * timestep ![dpm/Lts]

   d_A_234 = (desorbed_234 - adsorbed_234) * timestep  ![dpm/Lts]
   d_A_230 = (desorbed_230 - adsorbed_230) * timestep  ![dpm/Lts]

   p%Th_234 = p%Th_234 + dTh_p_234/p%n
   p%Th_230 = p%Th_230 + dTh_p_230/p%n

   A_Th_234 = A_Th_234 + d_A_234
   A_Th_230 = A_Th_230 + d_A_230

   Th_inv = Th_inv - dTh_p_234*p%n

   IF (p%Th_234 .lt. 0 .or. p%Th_230 .lt. 0) THEN
      print*, '------------------------------------------------'
      print*, adsorbed_234, desorbed_234
      print*, 'adsorb:', Th_k1, A_Th_234, p%n, mass
      print*, 'adsorb_230:', dTh_p_230, A_Th_230, p%Th_230, mass
   ENDIF

END SUBROUTINE adsorb
!========================================================================
SUBROUTINE adsorbtion_constant(p, k_1)
! k_1 is a function of size of particle
! r is in micro meters (as always) and the units of k_1 are:
! cm^3/g/d  where the cm^3/g refers to the volume of solution
! per gram of particle. -> lets change it to box/g
! formula from Burd and Jackson 2000
   USE the_info
   
   TYPE(agg_part), INTENT(INOUT) :: p
   DOUBLE PRECISION, INTENT(INOUT) :: k_1
 
   k_1 = (1000/p%r)**1.5 / 86400 * 1/(1e4*dz)

END SUBROUTINE adsorbtion_constant
!========================================================================
SUBROUTINE thorium_remin(gone, stuff, p)
   USE the_info
   
   TYPE(agg_part), INTENT(INOUT) :: p
   DOUBLE PRECISION, INTENT(INOUT) :: gone, stuff
   DOUBLE PRECISION :: organic, mineral, Th_234_lost, Th_230_lost, TEPC
   INTEGER :: i

   CALL find_depth(p%z, i)
 
   Th_234_lost = p%Th_234 * gone/stuff  !the fraction of Th remineralized
   Th_230_lost = p%Th_230 * gone/stuff  !the fraction of Th remineralized

   p%Th_234 = p%Th_234 - Th_lost_234
   Th_d_234(i) = Th_d_234(i) + Th_234_lost * p%n * 1/(10*dz) ![dpm/L]

   p%Th_230 = p%Th_230 - Th_lost_230
   Th_d_230(i) = Th_d_230(i) + Th_230_lost * p%n * 1/(10*dz) ![dpm/L]

   Th_inv = Th_inv + Th_234_lost*p%n

   IF (p%Th_234 .lt. 0) print*, 'thorium_remin', organic+minerals+TEPC, &
      Th_lost, p%Th_234, TEPC, p%id

IF (p%Th_234 .gt. 1 .or. p%Th_234 .lt. 0) print*, 'th_r', p%id, p%Th_234, p%r
IF (Th_d_234(i) .lt. 0) print*, 'thorium_remin:', Th_234_lost, Th_230_lost
IF (Th_d_230(i) .lt. 0) print*, 'thorium_remin:', Th_230_lost, i

END SUBROUTINE thorium_remin
!========================================================================
SUBROUTINE protists(mic, i, paraT, yaff)
   USE the_info
   101 FORMAT (E,1X,E,1X,E,1X,E,1X,E)

   INTEGER, INTENT(IN) :: i
   TYPE(zooplankton), INTENT(INOUT) :: mic
   DOUBLE PRECISION, INTENT(INOUT) :: paraT, yaff
   INTEGER :: max_i, x
   DOUBLE PRECISION :: depth

   depth = i*dz - 0.5*dz                ! depth [cm] at middle of box
...
!========================================================================
SUBROUTINE fecal_pellet(p, harvest, fracVar)
   USE the_info
...
!      p%Nn = FLOOR(-5e3*log(harvest))    try this!!!
      IF (p%Nn .eq. 0) p%Nn = 100
   ENDIF

   p%n = p%p/p%Nn
   p%Th_234 = p%Th_234*p%Nn/oldN
   p%Th_230 = p%Th_230*p%Nn/oldN
   i = 1
   DO WHILE (i .le. 10) 
   ...
   IF (ABS(test1-test2) .gt. 1) THEN
      ...
      print*, p%Nn, p%n, oldN
   ENDIF
   IF (ABS(before-after) .gt. 1) THEN
      print*, 'fecal pellet - Th', before, after, '!'
      print*, p%Nn, p%n, oldN
   ENDIF
   ...
   IF (p%Th_234 .gt. 1) print*, 'fp', p%id, p%Th_234, p%r
   IF (p%Th_234 .lt. 0) print*, 'Th - fp', p%id, p%Th_234, p%r
   IF (p%Th_230 .lt. 0) print*, 'Th - 230', p%id, p%Th_230
...
ENDIF
#ifdef thorium
   CALL thorium_remin(lostOrg, organic+TEPC, p)
#endif
...
!========================================================================
SUBROUTINE microzoop(p, zoop, harvest, paraT, paraD, fracVar, yaff)
   USE the_info

   TYPE(agg_part), INTENT(INOUT) :: p
   TYPE(zooplankton), INTENT(INOUT) :: zoop   !molC/m^3
   DOUBLE PRECISION, INTENT(INOUT) :: harvest, paraT, paraD, fracVar, yaff
   DOUBLE PRECISION :: zoo, dv, dv1, v, P_enc, orgCfrac, food, fact
   DOUBLE PRECISION :: P_break, apetite, microZ, depth, dummy, depth2
   INTEGER :: i, flag
...
!   fact = 1.0 !experiment1
!   fact = -7.0 ! experiment2
!   fact = -10+depth/500.0 !experiment3
!   P_enc = microZ * p%n * 10**fact * timestep/28800 
!   fact = fact - 4
!   fact = fact + yaff
!   fact = -10 + depth/yaff
...
!this is the one:!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!   depth2 = depth/15.0
!   fact = 0.17*(depth2**1.4*exp(-depth2/30.0))-10.0
!   P_enc = 10**fact * 1e8
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!   P_enc = microZ * p%n * 10**fact * timestep * yaff

   depth2 = depth/15.0
   fact = yaff*(depth2**0.5*exp(-depth2/80.0) - 3.9)
!   fact = -1 * yaff !experiment
   ...
   IF (P_enc .gt. harvest) THEN
      ...
      ELSEIF (apetite .gt. harvest2) THEN
         CALL ingestion(p, 1, paraT, paraD, harvest, fracVar)
      ENDIF
   ENDIF
 
END SUBROUTINE microzoop
!========================================================================
SUBROUTINE ingestion(p, flagFP, paraT, paraD, harvest, fracVar)
   USE the_info

   TYPE(agg_part), INTENT(INOUT) :: p
   DOUBLE PRECISION, INTENT(INOUT) :: paraT, paraD, fracVar
...
! zooplankton organic carbon and TEP respiration
 ...
 !zooplankton calcite dissolution
      IF (calcite .gt. 0) THEN
         ...
         ENDIF
#ifdef thorium
         CALL thorium_remin(lostC, calcite, p)
#endif
...
!         lostA = k_size*k_caco3_a*S_a*p%mineral(1)*timestep
         ...
         ENDIF
#ifdef thorium
         CALL thorium_remin(lostA, aragonite, p)
#endif
      ...
      ENDIF
   ENDIF
CALL fecal_pellet(p, harvest, fracVar)
...
!========================================================================
SUBROUTINE dissolution(p, paraC, paraT, fracVar)

   USE the_info

   TYPE(agg_part), INTENT(INOUT) :: p
   DOUBLE PRECISION, INTENT(INOUT) :: paraC, paraT, fracVar
   INTEGER :: x
...
!            lostC = k_size*k_caco3_c*S_c*p%mineral(1)*timestep
            ...
            ENDIF
#ifdef thorium
            CALL thorium_remin(lostC, p%mineral(1), p)
#endif
            p%mineral(1) = p%mineral(1) - lostC
            inventory(2) = inventory(2) - lostC*p%n
            lost(2) = lost(2) + lostC*p%n
...
!            lostA = k_size*k_caco3_a*S_a*p%mineral(2)*timestep
            ...
            ENDIF
#ifdef thorium
            CALL thorium_remin(lostA, p%mineral(2), p)
#endif
            p%mineral(2) = p%mineral(2) - lostA
            inventory(2) = inventory(2) - lostA*p%n
            lost(2) = lost(2) + lostA*p%n
...
! opal dissolution
      IF (p%mineral(3) .gt. 0) THEN
         k_size = (p%mineral(3)/(p%mineral(3)+1e-10))
         lostS = k_size*p%mineral(3)*Rd*timestep
#ifdef thorium
         CALL thorium_remin(lostS, p%mineral(3), p)
#endif
...
IF (p%Th_234 .lt. 0) print*, 'dissol - Th'
IF (p%Th_230 .lt. 0) print*, 'dissol - Th 230', p%z/100
